...
with open("var_results.json", "r") as var_file:
    ...
    # Persistency plot
    i = 0
    for d in list(data['Persistencies'].keys())[::-1]:
        plt.plot(x, data['Persistencies'][d].values(), label=format(Dens[i]) + "$\%$", color=colors[i], linewidth=2)
        i += 1
    ...
    i = 0
    for d in list(data['Trivial_decomposition'].keys())[::-1]:
        plt.plot(x, data['Trivial_decomposition'][d].values(), label=format(Dens[i]) + "$\%$", color=colors[i], linewidth=2)
        i += 1
    plt.xticks(ticks=np.arange(10, nodes_max, 20), labels=np.arange(10, nodes_max, 20))
    plt.xlabel('Number of nodes', fontsize=16)
    ...
    i = 0
    for d in list(data['CSCC_decomposition'].keys())[::-1]:
        plt.plot(x, data['CSCC_decomposition'][d].values(), label=format(Dens[i]) + "$\%$", color=colors[i], linewidth=2)
        i += 1
    plt.xticks(ticks=np.arange(10, nodes_max, 20), labels=np.arange(10, nodes_max, 20))
    plt.xlabel('Number of nodes', fontsize=16)
    plt.ylabel('Number of decompositions', fontsize=16)
    ...
    i = 0
    for d in list(data['Var_reduction'].keys())[::-1]:
        plt.plot(x, data['Var_reduction'][d].values(), label=format(Dens[i]) + "$\%$", color=colors[i], linewidth=2)
        i += 1
    plt.xticks(ticks=np.arange(10, nodes_max, 20), labels=np.arange(10, nodes_max, 20))
    plt.xlabel('Number of nodes', fontsize=16)
    ...
    for d in list(data['Var_reduction'].keys())[::-1]:
        ...
        patches.append(mpatches.Patch(color=colors[i], label=format(Dens[i]) + "$\%$"))
        i += 1
    ...
    plt.xlabel('Number of nodes', fontsize=16)
    plt.ylabel('Variable reduction $[\%]$', fontsize=16)
    plt.legend(title="Density", handles=patches, loc='lower left')
    ...
    patches = []
    i = 0
    for d in list(data['Shannon_sizes'].keys())[::-1]:
    ...
    for d in list(data['Shannon_sizes'].keys())[::-1]:
        ...
        plt.ylabel('Variable reduction $[\%]$', fontsize=16)
        plt.legend()
        # plt.savefig("Plots\\Reduction_comparison_P" + format(Dens[i]) + ".eps", format='eps', bbox_inches='tight')
        plt.savefig("Plots\\Reduction_comparison_P" + format(Dens[i]) + ".png", format='png', bbox_inches='tight')
        plt.savefig("Plots\\Reduction_comparison_P" + format(Dens[i]) + ".pdf", format='pdf', bbox_inches='tight')
        plt.close()
        i += 1
...
with open("bounds_results.json", "r") as bounds_file:
    ...
    colors = cmap(np.linspace(1, 0, 4))
    i = 0
    for d in list(data['Error'].keys())[::-1]:
        plt.plot(x, data['Error'][d].values(), label=format(Dens[i]) + "$\%$", linewidth=2, color=colors[i])
        i += 1
    plt.xticks(ticks=np.arange(10, nodes_max, 20), labels=np.arange(10, nodes_max, 20))
    plt.xlabel('Number of nodes', fontsize=16)
...
for item in os.listdir('data'):
    if item.endswith('_n.txt'):
        ex_density = int(item.split('P')[1].split('_')[0])
        n_nodes = int(item.split('_')[1])
        if ex_density not in average_d:
            average_d[ex_density] = []
            ...
            for line in f:
                ...
                for elem in splitted_line:
                    if elem != '\n':
                        if int(elem) > 0:
                            s += 1
            d = s / (n_nodes * (n_nodes - 1) / 2)
        average_d[ex_density].append(d)
for d in average_d.keys():
    average_d[d] = np.average(average_d[d])
print(average_d)
