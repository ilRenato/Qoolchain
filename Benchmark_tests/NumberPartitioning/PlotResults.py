...
for maxnum in M:
    try:
        resfile = open("NumPartStats_P{}.txt".format(maxnum), "r")
    except:
    ...
    for dim in range(MinProblemDimension, MaxProblemDimension + 1, 2):
        ...
        for n_prob in range(ProblemForEachDimension):
            try:
                sh_file = open("Results/NumberPartitioning_" + format(dim) + "_nodes_M" + format(maxnum) + "_" + format(n_prob) + "_n_sol.txt", "r")
            except:
...
for i in range(len(M)):
    plt.plot(persistency_list[i].keys(), persistency_list[i].values(), label="Density " + format(M[i]) + "%", linewidth=2)
...
for i in range(len(M)):
    plt.plot(trivial_dec_list[i].keys(), trivial_dec_list[i].values(), label="Density " + format(M[i]) + "%", linewidth=2)
plt.xlabel('Number of nodes', fontsize=15)
...
for i in range(len(M)):
    plt.plot(scc_dec_list[i].keys(), scc_dec_list[i].values(), label="Density " + format(M[i]) + "%", linewidth=2)
plt.xlabel('Number of nodes', fontsize=15)
plt.ylabel('Number of decompositions', fontsize=15)
...
for i in range(len(M)):
    plt.plot(var_reduction_list[i].keys(), var_reduction_list[i].values(), label="Density " + format(M[i]) + "%", linewidth=2)
plt.xlabel('Number of nodes', fontsize=15)
...
for i in range(len(M)):
    ...
    plt.ylabel('Variable reduction [%]', fontsize=15)
    plt.legend()
    plt.savefig("Reduction_comparison_P" + format(M[i]) + ".eps", format='eps', bbox_inches='tight')
    plt.savefig("Reduction_comparison_P" + format(M[i]) + ".png", format='png', bbox_inches='tight')
    plt.savefig("Reduction_comparison_P" + format(M[i]) + ".pdf", format='pdf', bbox_inches='tight')
    plt.close()
