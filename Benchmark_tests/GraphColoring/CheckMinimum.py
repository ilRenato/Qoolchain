...
opt_value_dict = {}
value_err_list = []
for filename in sorted(os.listdir("Results"), reverse=True):
    if not filename[-6:] == "_n.txt":
        ...
        for line in resfile:
            ...
            if splitted_line[0] == "Energy":
                if not flag:
                    ...
                    opt_value_dict[filename[:-10]] = opt_value
        ...
        resfile.close()
        err_dict[filename[:-10]] = (lower_bound - opt_value) * 100 / opt_value
        dwave_dict[filename[:-10]] = (dwave_dict[filename[:-10]] - opt_value) * 100 / opt_value
        ...
        err_bet_dict[filename[:-6]] = (range_bet - opt_value_dict[filename[:-6]]) * 100 / opt_value_dict[filename[:-6]]
        lower_wor, upper_wor = bounds_naive(qubo)
        range_wor = upper_wor - lower_wor
        err_wor_dict[filename[:-6]] = (range_wor - opt_value_dict[filename[:-6]]) * 100 / opt_value_dict[filename[:-6]]
