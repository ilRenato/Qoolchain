...
for prob in P:
    try:
        resfile = open("MinimumVertexCoverStats_P{}.txt".format(prob), "r")
    except:
    ...
    for dim in range(MinProblemDimension, MaxProblemDimension + 1, 2):
        ...
        for n_prob in range(ProblemForEachDimension):
            try:
                sh_file = open("Results/MinimumVertexCover_" + format(dim) + "_nodes_P" + format(prob) + "_" + format(n_prob) + "_n_sol.txt", "r")
            except:
...
# JSON FILE
try:
    outfile = open("jsons/var_results.json", 'w')
except:
    print("Error in json file creation\n")
    exit(1)
data = {'Persistencies' : {}, 'Trivial_decomposition' : {}, 'CSCC_decomposition' : {}, 'Var_reduction' : {}, 'Dwave' : {},
        'Shannon_sizes' : {}, 'Total_vars' : {}, 'Probing' : {}, 'All_persistencies' : {}}
for i in range(len(P)):
    data['Persistencies'][P[i]] = persistency_list[i]
    data['Trivial_decomposition'][P[i]] = trivial_dec_list[i]
    data['CSCC_decomposition'][P[i]] = scc_dec_list[i]
    data['Var_reduction'][P[i]] = var_reduction_list[i]
    data['Dwave'][P[i]] = dwave_list[i]
    data['Shannon_sizes'][P[i]] = shannon_sizes[i]
    data['Total_vars'][P[i]] = tot_var[i]
    data['Probing'][P[i]] = probing_list[i]
    data['All_persistencies'][P[i]] = all_pers_list[i]
json.dump(data, outfile)
outfile.close()

# Persistency plot
for i in range(len(P)):
    plt.plot(persistency_list[i].keys(), persistency_list[i].values(), label="Density "+format(P[i])+"%", color=colors[i], linewidth=2)
...
for i in range(len(P)):
    plt.plot(trivial_dec_list[i].keys(), trivial_dec_list[i].values(), label="Density "+format(P[i])+"%", color=colors[i], linewidth=2)
plt.xlabel('Number of nodes', fontsize=15)
...
for i in range(len(P)):
    plt.plot(scc_dec_list[i].keys(), scc_dec_list[i].values(), label="Density "+format(P[i])+"%", color=colors[i], linewidth=2)
plt.xlabel('Number of nodes', fontsize=15)
plt.ylabel('Number of decompositions', fontsize=15)
...
for i in range(len(P)):
    plt.plot(var_reduction_list[i].keys(), var_reduction_list[i].values(), label="Density "+format(P[i])+"%", color=colors[i], linewidth=2)
plt.xlabel('Number of nodes', fontsize=15)
